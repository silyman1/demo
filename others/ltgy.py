...
if __name__ == "__main__":
	csvfile = file('ltgy.csv','a')#ks.csv
	csvfile.write(codecs.BOM_UTF8)
	fo = open("ltgy.log",'w+')
	s = sys.stdout
	sys.stdout = fo
	origin_url = 'http://www.rubbertire.com.cn/gy/ltgy/ch/reader/issue_browser.aspx'
	urls_list = get_qikang_list(origin_url)
	flag =True
	for url in urls_list:
		astr = 'http://www.rubbertire.com.cn/gy/ltgy/ch/reader/'
		url = astr+url
		zurls_list = get_zhaiyao(url)
		for zurl in zurls_list:
			url = astr+zurl
			print '=============',url
			if url == 'http://www.rubbertire.com.cn/gy/ltgy/ch/reader/view_abstract.aspx?file_no=LTY20161209&flag=1':
				flag = False
				print 'go on'
			if flag:
				print 'pass'

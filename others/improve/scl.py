...
def get_detail2(rep):
	...
	someitems = []
	csvfile = file('gyscl233.csv','a')#ks.csv
	csvfile.write(codecs.BOM_UTF8)
	...
	for result in results:
		...
		if result.find('strong') == None:
			continue
		if result.find('strong').string == '通讯作者:':
		...
		if result.find('strong').string == '作者简介':
			print '有联系方式2'
			for thing in result.stripped_strings:
				print thing
				if thing.find('电话') ==-1 or thing.find('E-mail') ==-1 or thing.startswith('E-mail:'):
	...
	for page in range(2000,2019):
		...
		for url in urls_list:
			...
			for zurl in zurls_list:
				...
				print '=============',url
				# if url == 'http://www.iwt.cn/CN/abstract/abstract14123.shtml':
					# flag = False
					# print 'go on'
				# if flag:
					# print 'pass'
